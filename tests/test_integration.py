...
def ipython_magic(magic_line):
    magic, args = magic_line.split(None, 1)
    return ipython.run_line_magic(magic, args)
...
def test_function_not_defined(tmpdir, mock_key):
    ...
    with patch_ipython_globals(locals()):
        with raises(NameError, match="function 'pipe_delimited' is not defined in the global namespace"):
            ipython_magic('vault store x in my_frames with pipe_delimited')
...
def test_store(tmpdir):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in my_frames')
...
def test_store_import_del_using_path(tmpdir, mock_key):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in "my_frames/custom_path.tsv"')
    ...
    with patch_ipython_globals(globals()):
        ipython_magic('vault import "my_frames/custom_path.tsv" as y')

    assert_frame_equal(x, y, check_dtype=False)

    # TODO: fails on Windows, paths thing
    # ipython_magic('vault del "my_frames/custom_path.tsv"')

    # with raises(KeyError, match="There is no item named 'my_frames/custom_path.tsv' in the archive"):
    #     ipython_magic('vault import "my_frames/custom_path.tsv" as z')
...
def test_store_with_exporter(tmpdir):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in my_json with json.dump')
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in my_jars with pickle.dump')
        # ipython_magic('vault import x from my_jars as l with pickle.load')
        # assert x == l
...
def test_import_as(tmpdir, mock_key, secure):
    ipython_magic(f'open_vault --path {tmpdir}/archive.zip {secure}')
    x = EXAMPLE_DATA_FRAME
    ...
    with patch_ipython_globals(globals()):
        ipython_magic('vault import x from my_frames as y')
...
def test_del(tmpdir, mock_key, secure):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in my_frames')
        ipython_magic('vault del x from my_frames')

    with raises(KeyError, match="There is no item named 'my_frames/x' in the archive"):
        ipython_magic('vault import x from my_frames')
...
def test_del_not_in_namespace(tmpdir, mock_key, secure):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x as y in my_frames')
        ipython_magic('vault del y from my_frames')
...
def test_assert(tmpdir, mock_key, secure):
    ...
    with patch_ipython_globals(locals()):
        ...
        with raises(ValueError, match=r'Hash needs to have either 8 \(CRC32\) or 64 \(SHA256\) characters'):
            ipython_magic('vault assert x in my_frames is JH54')

        with raises(AssertionError):
            ipython_magic('vault assert x in my_frames is JH56321T')

        ipython_magic('vault assert x in my_frames is 3FDAA797')
        ipython_magic('vault assert x in my_frames is 3FDAA797 with CRC32')

        with raises(AssertionError):
            ipython_magic(f'vault assert x in my_frames is {"_" * 64} with SHA256')
...
def test_import_module(tmpdir, mock_key, secure):
    ...
    with patch_ipython_globals(locals()):
        ipython_magic('vault store x in my_frames')
        ipython_magic('vault store x in my_frames as y')

    with patch_ipython_globals(globals()):
        ipython_magic('vault import my_frames')
